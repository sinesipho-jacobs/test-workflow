...
def run_robot_tests():
    """Run Robot Framework tests, merge results, parse merged results, and post them to GitHub."""
    print(f"Current Working Directory: {os.getcwd()}")
    main_dir = './webapp_tests/robot-test-results'  # Directory where merged_output.xml is located
    merged_output_file = os.path.abspath('./webapp_tests/merged_output.xml')
    merged_log_file = os.path.abspath('./webapp_tests/merged_log.html')
    merged_report_file = os.path.abspath('./webapp_tests/merged_report.html')

    # Ensure merged output exists (which should be generated by rebot)
    if not os.path.isfile(merged_output_file):
        print(f"❌ Error: Merged output file '{merged_output_file}' not found.")
        return

    # Process the merged output.xml directly
    test_results = process_test_results(merged_output_file, merged_report_file)

    # Read the merged report content
    report_content = read_report_file(merged_report_file)
    post_github_check(test_results, report_content)


def merge_test_results(output_files, merged_output_file, merged_log_file, merged_report_file):
    """Merge Robot Framework output files using rebot."""
    print("Merging test results using rebot...")
    rebot_command = [
        "rebot", "--output", merged_output_file, "--log", merged_log_file,
        "--report", merged_report_file
    ] + output_files

    result = subprocess.run(rebot_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    if result.returncode != 0:
        print(f"❌ Error during rebot merge: {result.stderr.decode()}")
    else:
        print(f"✅ Successfully merged test results into {merged_output_file}, {merged_log_file}, and {merged_report_file}")


def process_test_results(output_file: str, report_file: str) -> dict:
    """Processes Robot Framework merged test results and returns structured data."""
...
def read_report_file(report_file: str) -> str:
    """Reads and returns the content of the merged report file."""
